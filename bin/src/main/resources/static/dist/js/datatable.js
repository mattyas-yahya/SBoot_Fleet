...
//    total = api.column( 2, { search: 'applied'} ).data().reduce( function (a, b)
//    {
//     return intVal(a) + intVal(b);
//    }, 0 );
    items = api.column( 4, { search: 'applied'} ).data().reduce( function (a, b)
    {
     return intVal(a) + intVal(b);
    }, 0 );

// Update footer
//$( api.column( 1 ).footer() ).html('Grand Total : Rp'+ total +' ');
    document.getElementById('valtotalitems').textContent = ' Total Items : '+ items +' ';;
 //  document.getElementById('valtotal').textContent = 'Grand Total : Rp'+ total +' ';;
...
$(document).ready( function ()
{
 tabeltransfer = $('#tabeltransfer').DataTable({
"columnDefs":[{ "orderable": false,"className": "select-checkbox","targets": 0,"checkboxes": {"selectRow": true}}],
"select": {"style": "multi"},
"order": [[1, "asc"]],
...
$('#nma').on( 'keyup click', function ()
{
$("#master").show();
tabelbrgmaster.search( this.value ).draw();
} );
